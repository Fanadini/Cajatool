...
| `indices.json` → `dolar` | Tipo de cambio minorista, promedio vendedor | Último dato (hoy $ 1.538,39 al 24/09/2026) | Automático, todos los lunes (`scripts/actualizar-indices.js`) | [BCRA](https://www.bcra.gob.ar/PublicacionesEstadisticas/Principales_variables.asp) · API variable 4 |
| `m2-caba.json` | Precio promedio de publicación del m² (USD) de departamentos en venta por barrio: 1 ambiente usado; 2 y 3 ambientes usados y a estrenar | 2.º trimestre de 2026 (provisorio) | Automático en el workflow semanal (`scripts/actualizar-m2.py`, requiere `openpyxl`). El IEC publica cada trimestre; si cambia la dirección del archivo (`uploads/2026/01/MI_DVP_AX01-04.xlsx` y `uploads/2025/01/MI_DVP_AX10.xlsx`), actualizarla en el script | [IEC CABA – Mercado inmobiliario](https://www.estadisticaciudad.gob.ar/eyc/categoria-banco-datos/mercado-inmobiliario/) |
...
| `/finanzas/dolar-tarjeta/` | Percepción del 30 % (RG ARCA 5617/2024) e IVA digital del 21 % están fijos en `calc.js`. Revisar si ARCA cambia el régimen | Si cambia la norma |
| Imágenes para compartir (`src/assets/img/og/`) | Se generan con `node scripts/generar-og.js` (requiere Playwright). Volver a correrlo al agregar una herramienta o cambiar su nombre | Al sumar herramientas |
